...
describe("rates hooks", () => {
  ...
  describe("useAllRates", () => {
    ...
    it("respects enabled option", () => {
      ...
      const { result: enabledResult } = renderHook(
        () => useAllRates({ enabled: false }),
        {
          wrapper: createWrapper(),
        }
      );

      expect(enabledResult.current.isLoading).toBe(false);
  ...
  describe("useCreateRate", () => {
    it("invalidates the correct query keys on success", async () => {
      ...
      await waitFor(() => {
        ...
        expect(invalidateQueriesSpy).toHaveBeenCalledWith({
          queryKey: adminKeys.studentRates(5),
        });
        expect(invalidateQueriesSpy).toHaveBeenCalledWith({
          queryKey: adminKeys.tutorRates(3),
        });
      });
  ...
  describe("useUpdateRate", () => {
    it("invalidates the correct query keys on success", async () => {
      ...
      await waitFor(() => {
        ...
        expect(invalidateQueriesSpy).toHaveBeenCalledWith({
          queryKey: adminKeys.studentRates(10),
        });
        expect(invalidateQueriesSpy).toHaveBeenCalledWith({
          queryKey: adminKeys.tutorRates(7),
        });
      });
    ...
    it("calls the correct service method", async () => {
      ...
      expect(mockRateService.updateRate).toHaveBeenCalledWith(1, { rate: 75 });
    });
  });
  ...
  describe("useDeleteRate", () => {
    it("invalidates only the rates query key on success", async () => {
      ...
      // Should only invalidate the general rates key, not specific student/tutor rates
